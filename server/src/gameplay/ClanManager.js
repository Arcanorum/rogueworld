...
const clanManager = {
    ...
    loadDataFromFile () {
        ...
        for(let i=0; i<data.length; i+=1){
            ...
            for(let j=0; j<clanData.structures.length; j+=1){
                ...
                new EntitiesList[structure.typeName]({row: structure.row, col: structure.col, board: charter.board, clan: charter.clan});
            }

        }

        console.log("* Loaded clans added.");
    },
    ...
    saveDataToFile () {
        ...
        if(Object.keys(this.clans).length === 0){
            console.log("* No clans data to save. Skipping.");
            return;
        }

        // For every clan that exists.
        for(let clanKey in this.clans){
            ...
            for(let structureKey in clan.structures){
                structures.push({
...
const boardsObject = require('../board/BoardsList').boardsObject;