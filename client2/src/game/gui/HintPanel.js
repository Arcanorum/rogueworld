...
class HintPanel extends PanelTemplate {
    ...
    show(panelNameTextDefID, contentTextDefID, contentFileName) {
        if (panelNameTextDefID === undefined) {
            this.changeName("");
        }
        else {
            this.changeName(window.dungeonz.getTextDef(`Hint panel: ${panelNameTextDefID}`));
        }

        if (contentTextDefID === undefined) {
            this.hintText.style.display = "none";
        }
        else {
            this.hintText.innerText = window.dungeonz.getTextDef(contentTextDefID);
