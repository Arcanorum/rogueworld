...
class Inventory {
    ...
    useHeldItem(direction) {
        if (direction === undefined) {
            ...
            window.ws.sendEvent("use_held_item");
        }
        else {
            // Tell the game server this player wants to use this item in a direction.
            window.ws.sendEvent("use_held_item", direction);
        }
    }
    ...
    useItem(slotNumber) {
        ...
        if (window.gameScene.keyboardKeys.shift.isDown === true) {
    ...
    swapInventorySlots(slotKeyFrom, slotKeyTo) {
        ...
        const GUIslots = window.gameScene.GUI.inventoryBar.slots;
        ...
        window.ws.sendEvent("swap_inventory_slots", { slotKeyFrom, slotKeyTo });
    }
}
