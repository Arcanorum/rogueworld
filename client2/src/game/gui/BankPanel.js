...
class Slot {
    ...
    refreshBackground() {
        ...
        const relativeSlotIndex = 1 * this.container.getAttribute("slotIndex")
            + window.gameScene.player.bankManager.selectedTabSlotIndexOffset;

        // Only give a white background back to the bank slots that have something in them.
        if (window.gameScene.player.bankManager.items[relativeSlotIndex].catalogueEntry === null) {
            this.container.style.backgroundColor = window.gameScene.GUI.GUIColours.bankSlotEmpty;
        }
        else {
            this.container.style.backgroundColor = window.gameScene.GUI.GUIColours.bankSlotOccupied;
        }
    }
    ...
    loadItem() {
        const relativeSlotIndex = 1 * this.container.getAttribute("slotIndex")
            + window.gameScene.player.bankManager.selectedTabSlotIndexOffset;
        /** @type {BankItem} */
        const bankItem = window.gameScene.player.bankManager.items[relativeSlotIndex];
...
class BankPanel extends PanelTemplate {
    constructor() {
        super(document.getElementById("bank_panel"), 500, 320,
            window.dungeonz.getTextDef("Bank panel: name"), "gui/panels/bank-chest");
    ...
    show() {
        ...
        window.gameScene.GUI.inventoryBar.showAddButtons();
    }
    ...
    hide() {
        ...
        window.gameScene.GUI.inventoryBar.hideAddButtons();
    }

    tabClick() {
        const tabNumber = this.getAttribute("tabNumber");
        window.gameScene.player.bankManager.loadTab(tabNumber);
        const { tabs } = window.gameScene.GUI.bankPanel;
        ...
        tabs[tabNumber].button.src = `assets/img/gui/panels/bank-tab-${tabNumber
        }-button-active.png`;
    }

    slotClick() {
        // console.log("slot clicked:", this.getAttribute('slotIndex'));
        const relativeSlotIndex = 1 * this.getAttribute("slotIndex")
            + window.gameScene.player.bankManager.selectedTabSlotIndexOffset;
        ...
        window.gameScene.player.bankManager.withdrawItem(relativeSlotIndex);
    }
    ...
    slotMouseOver() {
        const relativeSlotIndex = 1 * this.getAttribute("slotIndex")
            + window.gameScene.player.bankManager.selectedTabSlotIndexOffset;
        /** @type {BankItem} */
        const bankSlot = window.gameScene.player.bankManager.items[relativeSlotIndex];
        // If the slot is empty, don't show the tooltip.
        if (bankSlot.catalogueEntry === null) return;

        const { bankPanel } = window.gameScene.GUI;

        bankPanel.tooltip.innerText = window.dungeonz.getTextDef(
            `Item name: ${bankSlot.catalogueEntry.translationID}`,
        );
    ...
    slotMouseOut() {
        window.gameScene.GUI.bankPanel.tooltip.style.visibility = "hidden";
    }
    ...
    slotDragStart(event) {
        ...
        const relativeSlotIndex = 1 * slotIndex
            + window.gameScene.player.bankManager.selectedTabSlotIndexOffset;
        ...
        const { icon } = window.gameScene.GUI.bankPanel.slots[slotIndex];
        // console.log("icon:", icon);
        event.dataTransfer.setData("text/plain", null);
        window.gameScene.GUI.dragData = {
            dragOrigin: window.gameScene.GUI.bankPanel.bankSlots,
            bankSlot: window.gameScene.GUI.bankPanel.slots[slotIndex].container,
        };
        ...
        const bankGUISlots = window.gameScene.GUI.bankPanel.slots;
        for (let i = 0, len = bankGUISlots.length; i < len; i += 1) {
            bankGUISlots[i].container.style.backgroundColor = window.gameScene.GUI.GUIColours.validDropTargetOver;
        }

        // Highlight the inventory bar slots, to show that bank slots can be moved to the inventory bar.
        const inventoryGUISlots = window.gameScene.GUI.inventoryBar.slots;
        for (const slotKey in inventoryGUISlots) {
            if (inventoryGUISlots.hasOwnProperty(slotKey) === false) continue;
            inventoryGUISlots[slotKey].container.style.backgroundColor = window.gameScene.GUI.GUIColours.validDropTargetOver;
        }

        this.style.backgroundColor = window.gameScene.GUI.GUIColours.currentlyDragged;
    }
    ...
    slotDrop(event) {
        ...
        if (window.gameScene.GUI.dragData.dragOrigin === window.gameScene.GUI.inventoryBar.slotContainer) {
            const relativeSlotIndex = 1 * thisSlotIndex
                + window.gameScene.player.bankManager.selectedTabSlotIndexOffset;
            window.gameScene.player.bankManager.depositItem(window.gameScene.GUI.dragData.inventorySlot.slotKey,
                relativeSlotIndex);
        }
        else if (window.gameScene.GUI.dragData.dragOrigin === window.gameScene.GUI.bankPanel.bankSlots) {
            const otherSlotIndex = window.gameScene.GUI.dragData.bankSlot.getAttribute("slotIndex");
            window.gameScene.player.bankManager.swapSlots(otherSlotIndex, thisSlotIndex);
        }
        // Clear the drag origin, so other GUI elements don't still refer to the thing that was dragged when they are dropped over.
        window.gameScene.GUI.dragData.dragOrigin = null;
        // Reset the highlighted bank slots.
        const bankSlots = window.gameScene.GUI.bankPanel.slots;
    ...
    slotDragEnd(event) {
        ...
        const inventorySlots = window.gameScene.GUI.inventoryBar.slots;
