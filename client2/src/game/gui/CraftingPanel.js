...
class ComponentSlot {
    ...
    refreshBackground() {
        ...
        if (window.gameScene.craftingManager.components[this.container.getAttribute("slotKey")].occupiedBy
            === null) {
...
class CraftingPanel extends PanelTemplate {
    ...
    constructor(gui) {
        ...
        for (let i = 1; i < 6; i += 1) {
            ...
            this.components[`slot${i}`].remove = document.createElement("img");
    ...
    show(stationName, panelIconURL) {
        ...
        window.gameScene.craftingManager.recipeCode = "";
    ...
    hide() {
        ...
        window.gameScene.craftingManager.empty();
    ...
    slotClick() {
        ...
        window.gameScene.craftingManager.removeComponent(slotKey[4]);
    }
    ...
    slotMouseOver() {
        ...
        if (window.gameScene.craftingManager.components[slotKey].occupiedBy === null) return;

        const { craftingPanel } = window.gameScene.GUI;

        craftingPanel.tooltip.innerText = window.dungeonz.getTextDef(`Item name: ${
            window.gameScene.player.inventory[window.gameScene.craftingManager.components[slotKey].occupiedBy].catalogueEntry.translationID}`);
    ...
    slotMouseOut() {
        window.gameScene.GUI.craftingPanel.tooltip.style.visibility = "hidden";
    }
    ...
    componentsBarDrop(event) {
        ...
        if (window.gameScene.GUI.dragData.dragOrigin === window.gameScene.GUI.inventoryBar.slotContainer) {
            window.gameScene.craftingManager.addComponent(window.gameScene.GUI.dragData.inventorySlot.slotKey);
        }
    }
    ...
    resultMouseOver() {
        ...
        this.appendChild(window.gameScene.GUI.craftingPanel.tooltip);
    }
}
