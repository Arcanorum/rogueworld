...
class TaskSlot {
    ...
    constructor(panel, task) {
        ...
        let textName = window.dungeonz.getTextDef(`Task ID: ${task.taskID}`);
        ...
        if (task.rewardItemTypeNumbers[0]) {
            this.rewardItemIcon1.src = `assets/img/gui/items/${
                ItemTypes[task.rewardItemTypeNumbers[0]].iconSource}.png`;
        ...
        if (task.rewardItemTypeNumbers[1]) {
            this.rewardItemIcon2.src = `assets/img/gui/items/${
                ItemTypes[task.rewardItemTypeNumbers[1]].iconSource}.png`;
        ...
        if (task.rewardItemTypeNumbers[2]) {
            this.rewardItemIcon3.src = `assets/img/gui/items/${
                ItemTypes[task.rewardItemTypeNumbers[2]].iconSource}.png`;
    ...
    setDefault() {
        this.taskName.style.backgroundColor = window.gameScene.GUI.GUIColours.taskSlotDefault;
        this.progress.style.backgroundColor = window.gameScene.GUI.GUIColours.taskSlotDefault;
        this.rewardContainer.style.backgroundColor = window.gameScene.GUI.GUIColours.taskSlotDefault;
    }

    // setTracking () {
    //     this.taskName.style.backgroundColor = window.gameScene.GUI.GUIColours.taskSlotTracking;
    //     this.progress.style.backgroundColor = window.gameScene.GUI.GUIColours.taskSlotTracking;
    //     this.rewardContainer.style.backgroundColor = window.gameScene.GUI.GUIColours.taskSlotTracking;
    // }

    setCompleted() {
        this.taskName.style.backgroundColor = window.gameScene.GUI.GUIColours.taskSlotCompleted;
        this.progress.style.backgroundColor = window.gameScene.GUI.GUIColours.taskSlotCompleted;
        this.rewardContainer.style.backgroundColor = window.gameScene.GUI.GUIColours.taskSlotCompleted;
    }
}

class TasksPanel extends PanelTemplate {
    constructor(tasks) {
        super(document.getElementById("tasks_panel"), 540, 380, window.dungeonz.getTextDef("Tasks"),
            "gui/hud/tasks-icon");
        ...
        taskText.innerText = window.dungeonz.getTextDef("Task");
        const progressText = document.createElement("div");
        progressText.innerText = window.dungeonz.getTextDef("Progress");
        const rewardText = document.createElement("div");
        rewardText.innerText = window.dungeonz.getTextDef("Reward");
        ...
        trackText.innerText = window.dungeonz.getTextDef("Track");
        ...
        claimText.innerText = window.dungeonz.getTextDef("Claim");
    ...
    show() {
        ...
        for (const taskKey in this.taskSlots) {
    ...
    hide() {
        ...
        if (this.selectedSlot !== null) {
            this.selectedSlot.container.style.backgroundColor = window.gameScene.GUI.GUIColours.taskSlotUnselected;
            this.selectedSlot.container.style.borderColor = window.gameScene.GUI.GUIColours.taskSlotUnselected;
            this.selectedSlot = null;
        }
    ...
    rewardItemMouseOver() {
        const { tooltip } = window.gameScene.GUI.tasksPanel;
        ...
        if (ItemTypes[itemNumber]) {
            tooltip.innerText = window.dungeonz.getTextDef(
                `Item name: ${ItemTypes[this.getAttribute("itemNumber")].translationID}`,
            );
    ...
    rewardItemMouseOut() {
        window.gameScene.GUI.tasksPanel.tooltip.style.visibility = "hidden";
    }

    claimPressed() {
        const { tasksPanel } = window.gameScene.GUI;

        // Check a slot is actually selected.
        if (tasksPanel.selectedSlot === null) return;

        // Get the selected slot task ID.
        window.ws.sendEvent("task_claim_reward", tasksPanel.selectedSlot.container.getAttribute("taskID"));
     ...
     */
    addTask(task) {
        window.gameScene.player.tasks[task.taskID] = task;
    ...
    removeTask(taskID) {
        ...
        delete window.gameScene.player.tasks[taskID];
    }

    updateTaskProgress(taskID, progress) {
        window.gameScene.player.tasks[taskID].progress = progress;
        const taskSlot = this.taskSlots[taskID];
        taskSlot.progress.innerText = `${progress}/${
            window.gameScene.player.tasks[taskID].completionThreshold}`;
        // If the progress is now enough for the completion threshold, make the slot green.
        if (progress >= window.gameScene.player.tasks[taskID].completionThreshold) {
            taskSlot.setCompleted();
            taskSlot.completed = true;
            // Tell them via a chat message.
            window.gameScene.chat(undefined, window.dungeonz.getTextDef("Task completed"), "#50ff7f");
        }
    }
    ...
    slotClick() {
        ...
        if (tasksPanel.selectedSlot === null) {
            ...
            slot.container.style.backgroundColor = window.gameScene.GUI.GUIColours.taskSlotSelected;
            slot.container.style.borderColor = window.gameScene.GUI.GUIColours.taskSlotSelected;
            // Show the claim button if the task is complete.
            if (tasksPanel.selectedSlot.completed
                === true) {
                tasksPanel.claimButton.src = "assets/img/gui/panels/claim-button-border-valid.png";
            }
            else {
                tasksPanel.claimButton.src = "assets/img/gui/panels/claim-button-border-invalid.png";
            }
        ...
        // The selected slot was selected again, deselect it.
            ...
            tasksPanel.selectedSlot.container.style.backgroundColor = window.gameScene.GUI.GUIColours.taskSlotUnselected;
            tasksPanel.selectedSlot.container.style.borderColor = window.gameScene.GUI.GUIColours.taskSlotUnselected;
